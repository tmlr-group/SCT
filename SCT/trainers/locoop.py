...
class LoCoOp(TrainerX):
    ...
    def test_ood(self, data_loader, T, id_flag):
        ...
        if id_flag:
            return concat(mcm_score)[:len(data_loader.dataset)].copy(), concat(glmcm_score)[:len(
                data_loader.dataset)].copy(), correct / len(data_loader.dataset)
        else:
            return concat(mcm_score)[:len(data_loader.dataset)].copy(), concat(glmcm_score)[:len(
                data_loader.dataset)].copy()
